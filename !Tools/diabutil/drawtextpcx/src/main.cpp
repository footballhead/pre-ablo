...
int main(int argc, char **argv)
{
	...
	drpcx_uint8* pcx_data = drpcx_load_file(in_pcx_file, DRPCX_FALSE, &pcx_width, &pcx_height, &pcx_components, 0);
	if (pcx_data == NULL) {
	...
	std::vector<image_t> glyphs(num_frames, make_blank_image(pcx_width, space_height));
	for (auto glyph_index = 0; glyph_index < num_frames; ++glyph_index) {
		...
		for (auto i = 0; i < glyph_area * pcx_components; i += pcx_components) {
			auto color = color_t{glpyh_data_base[i], glpyh_data_base[i+1], glpyh_data_base[i+2], 255};
