...
constexpr auto mfontkern = std::array<uint8_t, 56>{ 18,										// Space/Invalid
...
int main(int argc, char **argv)
{
	...
	if (celparts.size() + 1 != mfontkern.size()) {
		fprintf(stderr, "Expected %zu characters, got %zu\n", mfontkern.size(), celparts.size());
		return 1;
	}

	std::vector<image_t> glyphs{};
	glyphs.reserve(celparts.size());
	std::transform(cbegin(celparts), cend(celparts), std::back_inserter(glyphs), [&palette](cel_data const& data) {
		return image_from_cel(data, BigTGold_width, palette, /*has_header=*/false);
	});
