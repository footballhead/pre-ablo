...
namespace std {
	template<> struct hash<mini_tile> {
		std::size_t operator()(mini_tile const& mt) const noexcept
		{
			return mt.data;
		}
	};
...
int main(int argc, char** argv)
{
	if (argc != 2) {
		...
		fprintf(stderr, "Mutate a celpart so that it can be displayed properly by prdemo\n");
		return 1;
	}
	...
	for (int i = 0; i < min_file_contents.size(); i += 2) {
		uint16_t buffer = *reinterpret_cast<uint16_t const*>(min_file_contents.data() + i);
