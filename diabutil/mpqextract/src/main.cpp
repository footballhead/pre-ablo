...
namespace
{
    ...
    {
        for (auto place = str.find(to_replace); place != std::string::npos; place = str.find(to_replace))
        {
    ...
    {
        for (auto place = str.find(to_remove); place != std::string::npos; place = str.find(to_remove))
        {
    ...
    void print_usage(char *const program_name)
    {
        std::cerr << "Usage: " << program_name << " file.mpq\n"
                  << "Reads listfile from stdin then, for each line in stdin, extracts that file relative to the working directory.\n"
                  << "\n"
...
int main(int argc, char **argv)
{
    ...
    {
        std::cerr << "Failed to open MPQ: err=" << GetLastError() << " file=" << mpq_filename << '\n';
        return 1;
    }
    ...
    {
        // This is more for my sanity (especially on Docker with Windows files mounted)
        line = remove_all(line, "\r");
        // listfiles typically have Windows paths, but unix-style paths are more widely accepted
        ...
        if (!std::filesystem::exists(localdir))
        {
            ...
            {
                std::cerr << "Failed to create directory. Continuing... localdir=" << localdir << '\n';
                continue;
            }
        }
        ...
        if (!SFileExtractFile(mpq, line.c_str(), line.c_str(), SFILE_OPEN_FROM_MPQ))
        {
